...
async def test_update_data_with_gas(auth_mock):
    ...
    with patch.object(coord_mod.dt_util, "utcnow", return_value=mock_now), \
         patch.object(coordinator, "_async_fetch_data", new_callable=AsyncMock) as mock_fetch, \
         patch.object(coordinator, "_insert_statistics", new_callable=AsyncMock) as mock_insert:

        async def mock_fetch_side_effect(url, token):
            if "electricity/cache" in url and "interval=HOUR" in url:
                return {"data": [{"startDate": "2026-04-20T12:00:00.000Z", "usage": 1.5, "vasteKosten": {"abonnementsKosten": 2.0, "netbeheerKosten": 1.0}}]}
            elif "electricity/cache" in url and "interval=MONTH" in url:
                return {"data": [{"startDate": "2026-04-01T00:00:00.000Z", "usage": 100.0}]}
            elif "production/cache" in url and "interval=HOUR" in url:
                return {"data": [{"startDate": "2026-04-20T12:00:00.000Z", "usage": 0.5}]}
            elif "production/cache" in url and "interval=MONTH" in url:
                return {"data": [{"startDate": "2026-04-01T00:00:00.000Z", "usage": 50.0}]}
            elif "gas/cache" in url and "interval=HOUR" in url:
                return {"data": [{"startDate": "2026-04-20T12:00:00.000Z", "usage": 2.5, "vasteKosten": {"abonnementsKosten": 3.0}}]}
